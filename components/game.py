...
class Game:
    ...
    def get_order_player(self):
        ...
        while True:
            player_rolls = list(map(lambda player: (player, self.dice.roll()), self.players))
            ...
            tied_players = [player_roll[0] for player_roll in player_rolls if player_roll[1] == highest_roll]
            
            if len(tied_players) == 1:
                return [roll[0] for roll in player_rolls]
            ...
            if len(tied_players) == 1:
                ...
                clear()
                return order            
            # for index, (player, roll) in enumerate(player_rolls):
            #     if roll == highest_roll:
            #         player_rolls[index] = (player, self.dice.roll())
